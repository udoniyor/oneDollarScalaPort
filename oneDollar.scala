/*The $1 Unistroke Recognizer (Scala Version)
 * 
 ...
 *  du26@sussex.ac.uk
 * 
 * The academic publication for the $1 recognizer:
 ...
 *   
 * The Protractor enhancement was separately published by Yang Li:
...
class recognizer() {
  ...
  private def pathLength(ls: List[Point]): Double = ls match {
    case Nil => 0.0
    case head :: tail => if (tail.length > 0) distance(head, tail.head) + pathLength(tail) else 0.0
  }

  private def pathsDistanceTotal(ls: List[Point], lz: List[Point]): Double = ls match {
    case Nil => 0.0
    case head :: tail => if (tail.length > 0) distance(head, lz.head) + pathsDistanceTotal(tail, lz.tail) else 0.0
  }

  private def pathsDistance(ls: List[Point], lz: List[Point]): Double = pathsDistanceTotal(ls, lz) / ls.length

  private def boundingBox(ls: List[Point]): Rectangle = {
    val minX = ls.minBy(_.x).x
    val minY = ls.minBy(_.y).y
    val maxX = ls.maxBy(_.x).x
    val maxY = ls.maxBy(_.y).y
    new Rectangle(minX, minY, maxX - minX, maxY - minY)
  }

  private def centroid(ls: List[Point]): Point = {
    val x = sumPoints((w: Point) => w.x)(ls) / ls.length
    val y = sumPoints((w: Point) => w.y)(ls) / ls.length
    new Point(x, y)
  }

  private def sumPoints(f: Point => Double)(ls: List[Point]): Double = ls match {
    case Nil => 0.0
    case head :: tail => f(head) + sumPoints(f)(tail)
  }

  private def vectorize(ls: List[Point]): List[Double] = {
    val flatList = extractList(ls)
    val sum = flatList.foldLeft(0.0)((r, c) => r + c * c)
    val mag = Math.sqrt(sum)
    flatList map (x => x / mag)
  }

  private def extractList(ls: List[Point]): List[Double] = ls match {
    case Nil => Nil
    case head :: tail => List(head.x, head.y) ++ extractList(tail)
  }

  private def optimalCosineDistance(p1: List[Double], p2: List[Double]): Double = {
    val a = diffTwo((w: Double) => w)(p1, p2)
    val b = diffTwo((w: Double) => -w)(p1, p2)
    val angle = Math.atan(b / a)
    Math.acos(a * Math.cos(angle) + b * Math.sin(angle))
  }

  private def diffTwo(f: Double => Double)(ls: List[Double], lz: List[Double]): Double = ls match {
    case Nil => 0.0
    case head :: tail => head * lz.head + f(diffTwo(f)(tail, lz.tail))
  }

  private def translateTo(ls: List[Point], pt: Point): List[Point] = {
    val c = centroid(ls)
    ls map (x => x + pt - c)
  }

  private def scaleTo(ls: List[Point], size: Double): List[Point] = {
    val b = boundingBox(ls)
    val divider = new Point(size / b.width, size / b.height)
    ls map (x => x * divider)
  }

  private def rotateBy(ls: List[Point], rad: Double): List[Point] = {
    val c = centroid(ls)
    val cos = Math.cos(rad);
    val sin = Math.sin(rad);
    ls map (x => x.rotate(c, cos, sin))
  }

  private def indicativeAngle(ls: List[Point]): Double = {
    val c = centroid(ls)
    Math.atan2((c - ls.head).y, (c - ls.head).x)
  }

  private def distanceAtAngle(ls: List[Point], lz: List[Point], rad: Double): Double = pathsDistance(rotateBy(ls, rad), lz)

  private def distanceAtBestAngle(ls: List[Point], lz: List[Point], negativeAngleR: Double, positiveAngleR: Double, threshold: Double): Double = {
    var x1 = Phi * negativeAngleR + (1.0 - Phi) * positiveAngleR;
    var f1 = distanceAtAngle(ls, lz, x1);
    var x2 = (1.0 - Phi) * negativeAngleR + Phi * positiveAngleR;
    var f2 = distanceAtAngle(ls, lz, x2);
    var w = positiveAngleR
    var z = negativeAngleR
    while (Math.abs(w - z) > threshold) {
      if (f1 < f2) {
        w = x2
        x2 = x1;
        f2 = f1;
        x1 = Phi * negativeAngleR + (1.0 - Phi) * positiveAngleR;
        f1 = distanceAtAngle(ls, lz, x1);
      } else {
        z = x1;
        x1 = x2;
        f1 = f2;
        x2 = (1.0 - Phi) * negativeAngleR + Phi * positiveAngleR;
        f2 = distanceAtAngle(ls, lz, x2);
      }
    }
    Math.min(f1, f2);
  }

  private def reSamplePoints(ls: List[Point], lz: List[Point], interval: Double, D: Double): List[Point] = ls match {
    case Nil => lz
    case head :: tail => {
      if (tail.length > 0) {
        val pt1 = head
        ...
        if (D + d >= interval) {
          val qx = pt1.x + ((interval - D) / d) * (pt2.x - pt1.x)
          val qy = pt1.y + ((interval - D) / d) * (pt2.y - pt1.y)
          val q = List(new Point(qx, qy))
          reSamplePoints(q ++ tail, lz ++ q, interval, 0.0)
        } else { reSamplePoints(tail, lz, interval, D + d) }
      } else lz ++ List(head)
    }
  }

  private def reSample(points: List[Point], n: Int): List[Point] = {
    val interval = pathLength(points) / (n - 1.0)
    val lz = List()
    val finalL = reSamplePoints(points.reverse, lz, interval, 0.0).reverse
    if (finalL.length == (n - 1)) {
      finalL ++ List(points.reverse.head)
    } else { finalL }
  }
}
